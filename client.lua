...
function resetJobState()
    ...
    if (postalJobState.goPostalVan) then
        ...
        print('^6[^3rt-postal^6]^0 Unsupported Target detected!')
        end
    end
...
function startPostalJob()
    ...
    TriggerServerEvent("rt-postal:server:start:job")
    ...
    postalJobState.postalBoxZone = TARGET.AddBoxZone('rt-postal-grab-delivery', deliveryCoords, vec3(1.0, 1.0, 3.0), parameters)
...
function endPostalJob()
    ...
    TriggerServerEvent("rt-postal:server:end:job")
...
function deliverPackageToPed()
    ...
    TriggerServerEvent('rt-postal:server:compensateDelivery', postalJobState.positionSet)
